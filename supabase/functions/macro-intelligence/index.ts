...
serve(async (req) => {
  ...
  try {
    ...
    if (!Deno.env.get("OPENROUTER_API_KEY")) return new Response(JSON.stringify({ error: "AI service not configured" }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
